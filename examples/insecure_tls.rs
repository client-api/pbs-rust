...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let host = env::var("PBS_HOST").unwrap_or_else(|_| "https://localhost:8007".into());
    ...
    let target = TerminalTarget::Qemu {
        node: env::var("PBS_NODE").unwrap_or_else(|_| "pbs1".into()),
        vmid: env::var("PBS_VMID").unwrap_or_else(|_| "100".into()).parse()?,
    };
...
impl AuthAttacher for DefaultAuthForExample {
    fn apply(
        ...
        if let Some(token) = cfg.bearer_access_token.as_ref() {
            let header =
                tokio_tungstenite::tungstenite::http::HeaderValue::from_str(&format!("PBSAPIToken={}", token))?;
