// SC-20..22 — authorization on PBS.

mod common;

use clientapi_pbs::apis::configuration::ApiKey;
use clientapi_pbs::apis::{access_acl_api, access_api, access_users_api, Error};
...
const READONLY_USER: &str = "e2e-readonly@pbs";
const READONLY_TOKEN_ID: &str = "probe";
const ADMIN_PROBE_USER: &str = "e2e-admin-probe@pbs";
...
async fn sc_20_readonly_user_cannot_create() {
    ...
    // PBS uses `Audit` as the read-only role and `auth_id` (single string)
    // rather than PVE's separate `users`/`groups`/`tokens` columns.
    ...
    readonly_cfg.api_key = Some(ApiKey {
        prefix: None,
        key: format!(
            "PBSAPIToken={READONLY_USER}!{READONLY_TOKEN_ID}:{value}",
    ...
    let err = access_users_api::access_users_create_users(
        &readonly_cfg,
        AccessUsersCreateUsersRequest::new("e2e-blocked@pbs".to_string()),
    )
    .await
...
async fn sc_22_permissions_returns_effective_acls() {
    ...
    // PBS uses `auth_id` query param, not `userid`.
